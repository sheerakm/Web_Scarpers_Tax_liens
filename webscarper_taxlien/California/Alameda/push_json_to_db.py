...
Auction_start = datetime(2025, 3, 18, 8, 0, 0, tzinfo=timezone(timedelta(hours=-7)))
Auction_end = datetime(2025, 3, 21, 8, 0, 0, tzinfo=timezone(timedelta(hours=-7)))
...
def push_parcels_from_file(json_file_path):
    ...
    for parcel in parcels:
        ...
        parts = [parcel["Property Address"] , ' ' , parcel['City'] ,' ' , parcel['Zip'] ,' ' ,  county_name]

        mapped_data['Address'] = " ".join([str(p) for p in parts if p])
