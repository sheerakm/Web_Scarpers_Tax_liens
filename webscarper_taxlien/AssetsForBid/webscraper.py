        ...
        for auction in auctions:
            try:
                ...
                if link_element:
                    ...
                    print(f" - Visiting: {name} ({link})")

                    if link:
                        print(f"Opening link: {link}")
                        link_element.click()  # Click the link to open it

                        # Wait for the new page to load (by waiting for an element on that page)
                        WebDriverWait(driver, 10).until(
                            EC.presence_of_element_located((By.TAG_NAME, "h1"))  # Example: Wait for an <h1> element
                        )
